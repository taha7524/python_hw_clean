...
def test_create_project_negative():
    ...
    # создание проекта
    result = api.create_project('', users)
    ...
    assert result["message"][0] == "title should not be empty"
...
def test_get_project_with_id_negative():
    ...
    # обращаемся к проекту
    new_project = api.get_project_with_id("несуществующий проект")

    assert new_project['message'] == "Проект не найден"
    assert new_project['statusCode'] == 404



def test_edit_project_negative():

    result = api.edit_project("несуществующий проект", 
                              False, 'Edited_Project_Test')
